...
function Menu() {
  ...
  const TempCategories = useSelector(state => state?.categories.categoriesDate);
  ...
  return ( 
    ...
    <div className="categories" id='menuCategories'>
      <ul>
        {categories?.map((item) => (
          <li key={item.id}>
            <a id="categoriesType"href={`#${item.name}`}>{item.name}</a>
          </li>
        ))}
      </ul>
      ...
      animate={{ x: 0, transition: { type: "spring", duration: 2 } }}
            key={itemType.id} className='mealsCard'
